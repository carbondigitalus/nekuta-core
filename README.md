...
src/
  ...
  react/
    context.tsx        ← NekutaProvider — replaces provide/inject; also SSR request-scoping boundary
...
- `gulp release` (composed of `release:version`, `release:changelog`, `release:publish`, `release:tag` via `gulp.series`) — port of `pinia/scripts/release.ts`, simplified since there are only two publishable packages here (`@nekuta/core`, `@nekuta/next`) versus Pinia's three. Each package gets independent versioning/changelog via `conventional-changelog -p angular --commit-path apps/<pkg>`, matching Pinia's own per-package pattern. Should call the repo's existing-but-unused `.github/npm-version-script.js` (a beta-version-bump-by-branch-name script already present) for prerelease branches rather than reimplementing that logic.
